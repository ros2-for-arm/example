...
static TEE_OperationHandle aes_encrypt_op_handle[TA_PUBLIC_MAX_KEY_STORAGE];
static TEE_OperationHandle aes_decrypt_op_handle[TA_PUBLIC_MAX_KEY_STORAGE];
static TEE_OperationHandle hmac_op_handle[TA_PUBLIC_MAX_KEY_STORAGE];
             ...
             uint32_t* size_buff_out)
{
  ...
  result = ta_aes_initialize_key(aes_secret,
           size_aes_secret,
           key_size_bit,
           &(aes_encrypt_op_handle[id]),
           &(aes_decrypt_op_handle[id]));
  if (result != TEE_SUCCESS) {
    goto error;
  }
             ...
             void* buff_out, uint32_t* size_buff_out)
{
  ...
  TEE_MACInit(hmac_op_handle[id], (void*) NULL, 0);
  result = TEE_MACComputeFinal(hmac_op_handle[id],
                buff_in,
                size_buff_in,
                buff_out,
                size_buff_out);
                               ...
                               uint32_t size_sha_in)
{
  ...
  TEE_MACInit(hmac_op_handle[id], (void*) NULL, 0);
  result = TEE_MACCompareFinal(hmac_op_handle[id],
            buff_in,
            size_buff_in,
            sha_in,
            size_sha_in
                          ...
                          void* iv, uint32_t* iv_size_byte)
{
  ...
  *iv_size_byte = TA_AES128_SIZE_BYTE;
  ...
  TEE_CipherInit(aes_encrypt_op_handle[id], iv, *iv_size_byte);

  // Write the size of the message
  result = TEE_CipherUpdate(aes_encrypt_op_handle[id],
       (void*) &size_buff_in, 4,
       buff_out,
       size_buff_out
      );
  ...
  // Add the message
  result = TEE_CipherDoFinal(aes_encrypt_op_handle[id],
        buff_in,
        size_buff_in,
        buff_out,
        size_buff_out);
                          ...
                          void* iv, uint32_t iv_size_byte)
{
  ...
  TEE_CipherInit(aes_decrypt_op_handle[id], iv, iv_size_byte);
  result = TEE_CipherDoFinal(aes_decrypt_op_handle[id],
        buff_in,
        size_buff_in,
        buff_out,
        size_buff_out);
