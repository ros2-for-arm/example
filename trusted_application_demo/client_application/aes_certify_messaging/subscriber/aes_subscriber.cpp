...
public:
  ...
  : Node("subscriber_and_client") {
    ...
    teec_exitOnFailure(&ctx, &sess,
                       aes_createKeyFromEncryptedSecret(&sess,
                                                        &aes_encrypted_key_shm,
                                                        aes_encrypted_key_shm.size)
                      );

    // Subscribe to the corresponding topic
    subscription = this->create_subscription<AesMessageCertify>(
      topic_name, std::bind(&AesSubscriber::topic_callback, this, _1));

    RCLCPP_INFO(this->get_logger(),
      "Subscriber is fully initialized with \n"
...
private:
    ...
    while(!validated){
                ...
                answer_service) != rclcpp::executor::FutureReturnCode::SUCCESS)
      {
        RCLCPP_ERROR(this->get_logger(),
         "Something happened, service could not deliver the secret... Exiting");
        teec_closeSession(&ctx, &sess);
        exit(-1);
      }
      ...
      validated = rsa_verify(&sess,
                             &aes_encrypted_key_shm,
                             aes_encrypted_key_shm.size,
                             &sha1_rsa_signed_shm,
                             sha1_rsa_signed_shm.size);
    }
  }
  ...
  void topic_callback(const AesMessageCertify::SharedPtr msg) {
    ...
    for(i = 0; i < msg->message.size() && i < clear_message_shm.size; i++){
    ...
    for(i = 0; i < msg->sha.size() && i < sha_shm.size; i++){
      sha_cast[i] = msg->sha[i];
    }

    // Compare the digest of the message with the actual digest sent
    if(hmac_compareSHA(&sess, &clear_message_shm, &sha_shm, msg->message.size())){
